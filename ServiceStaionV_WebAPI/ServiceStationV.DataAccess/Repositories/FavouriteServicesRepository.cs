...
{
    public class FavouriteServicesRepository : IFavouriteServicesRepository
    {
        private ServiceStationVDBContext _context;
        public FavouriteServicesRepository(ServiceStationVDBContext context)
        {
            _context = context;
        }
...
if (!serviceExists)
{
    throw new InvalidOperationException("Такой услуги не существует");
            ...
            if (exists)
            {
                throw new InvalidOperationException("Эта услуга уже в избранном");
            }
